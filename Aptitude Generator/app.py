...
try:
    ...
    model_name = next((model for model in preferred_models if f'models/{model}' in available_models), None)
    if model_name:
...
# Fallback questions for domain-specific
...
def generate_domain_questions(domain, num_questions=5):
    is_supported_domain = domain in supported_domains
    prompt = f"""
    ...
    try:
        ...
        if len(parsed_questions) >= num_questions:
            ...
            if is_supported_domain:
                ...
                fallback_questions = random.sample(fallback_domain_questions[domain], min(shortfall, len(fallback_domain_questions[domain])))
        ...
        if is_supported_domain:
            print("Using fallback questions.")
            fallback_questions = random.sample(fallback_domain_questions[domain], min(num_questions, len(fallback_domain_questions[domain])))
...
def generate_verbal_questions(num_questions=5):
    ...
    try:
        ...
        if len(parsed_questions) >= num_questions:
            ...
            shortfall = num_questions - len(parsed_questions)
            fallback_questions = random.sample(fallback_verbal_questions, min(shortfall, len(fallback_verbal_questions)))
            return parsed_questions + fallback_questions[:shortfall]
    except Exception as e:
        print(f"Failed to generate Verbal Communication questions: {e}. Using fallback.")
        fallback_questions = random.sample(fallback_verbal_questions, min(num_questions, len(fallback_verbal_questions)))
        return fallback_questions
...
def start_test():
    ...
    if not domain:
        return jsonify({'error': 'Please select or enter a valid domain (e.g., ISE)'}), 400
    
    session['domain'] = domain
    
    general_aptitude_questions = load_questions_from_csv('clean_general_aptitude_dataset.csv', 'General Aptitude')
    logical_reasoning_questions = load_questions_from_csv('logical_reasoning_questions.csv', 'Logical Reasoning')
    
    general_questions = random.sample(general_aptitude_questions, 10) if len(general_aptitude_questions) >= 10 else general_aptitude_questions
    logical_questions = random.sample(logical_reasoning_questions, 5) if len(logical_reasoning_questions) >= 5 else logical_reasoning_questions
